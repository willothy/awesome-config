...
editor_cmd = (editor:match("emacs -nw") or not editor:match("emacs")) and terminal .. term_cmd .. editor or editor
...
awful.keyboard.append_global_keybindings({
	...
	------------
	-- Reload Awesome
	awful.key({ modkey, "Control" }, "r", awesome.restart, { description = "Reload AwesomeWM", group = "AwesomeWM" }),
	-- Awesome Menu
	awful.key({ modkey }, "w", function()
		menu.mainmenu:toggle()
	end, { description = "Toggle Awesome Menu", group = "AwesomeWM" }),
	...
	end, { description = "Opens a terminal scratchpad", group = "Programs" }),
	-- File Browsers (term, gui)
	awful.key({ modkey }, "e", function()
		awful.spawn(files_cmd)
	end, { description = "Open a TUI file manager", group = "Programs" }),
	awful.key({ modkey, "Shift" }, "e", function()
		awful.spawn(files_gui)
	end, { description = "Open a GUI file manager", group = "Programs" }),
	-- Task Manager
	awful.key({ modkey }, "Escape", function()
		awful.spawn(tasks_cmd)
	end, { description = "Open a task manager", group = "Programs" }),
	...
	awful.key({ modkey }, "Tab", function()
		awful.layout.inc(1)
	end, { description = "Switch to next layout", group = "Layout Management" }),
	awful.key({ modkey, "Shift" }, "Tab", function()
		awful.layout.inc(-1)
	end, { description = "Switch to previous layout", group = "Layout Management" }),
	...
	end, { description = "Increase system audio volume", group = "Media" }),
	awful.key({ nil }, "XF86AudioLowerVolume", function()
		awesome.emit_signal("volume::change", -5)
	end, { description = "Decrease system audio volume", group = "Media" }),
	awful.key({ nil }, "XF86AudioMute", function()
		awesome.emit_signal("volume::mute")
	end, { description = "Mute system audio", group = "Media" }),
	-- song playback (play, prev, next)
	awful.key({ nil }, "XF86AudioPlay", function()
		playerctl:play_pause()
	end, { description = "Play/pause media playback", group = "Media" }),
	awful.key({ nil }, "XF86AudioPrev", function()
		playerctl:previous()
	end, { description = "Skip to next song/video", group = "Media" }),
	awful.key({ nil }, "XF86AudioNext", function()
		playerctl:next()
	end, { description = "Rewind to previous song/video", group = "Media" }),
	-- brightness (up, down)
	awful.key({ nil }, "XF86MonBrightnessUp", function()
		awesome.emit_signal("brightness::change", 5)
	end, { description = "Increase screen brightness", group = "Media" }),
	awful.key({ nil }, "XF86MonBrightnessDown", function()
		awesome.emit_signal("brightness::change", -5)
	end, { description = "Decrease screen brightness", group = "Media" }),
	-- language
	awful.key({ modkey }, "u", function()
		awesome.emit_signal("signal::lang")
	end, { description = "Cycle keyboard layouts", group = "Media" }),
	-- screenshot (selection, screen)
	awful.key({}, "Print", function()
		awesome.emit_signal("screenshot::selection")
	end),
	awful.key({ modkey }, "Print", function()
		awesome.emit_signal("screenshot::fullscreen")
	end),
	...
	awful.key({ modkey }, "b", function()
		awesome.emit_signal("widget::bar")
	end, { description = "Toggle bar visibility", group = "UI" }),
	-- toggle dashboard
	awful.key({ modkey }, "d", function()
		awesome.emit_signal("widget::dashboard")
	end, { description = "Toggle dashboard visibility", group = "UI" }),
})
...
client.connect_signal("request::default_keybindings", function()
	awful.keyboard.append_client_keybindings({
		...
		awful.key({ modkey }, "f", function(c)
			...
			c:raise()
		end, { description = "Toggle fullscreen client", group = "Window Management" }),
		awful.key({ modkey }, "m", function(c)
			c.maximized = not c.maximized
			c:raise()
		end, { description = "Toggle maximized client", group = "Window Management" }),
		awful.key(
			{ modkey },
			"s",
		...
		end, { description = "Toggle sticky client", group = "Window Management" }),
		-- default close window behaviour
		awful.key({ modkey }, "q", function(c)
			c:kill()
		end, { description = "Close client", group = "Window Management" }),
