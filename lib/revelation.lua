...
type(timer) ~= "table" and require("gears.timer").delayed_call
)
...
if type(awful.client.object) == "table" then
  view_only_func = function(tag)
    tag:view_only()
  end
  toggle_tag_func = function(t, c)
    c:toggle_tag(t)
  end
  jump_to_func = function(c)
    c:jump_to()
  end
else
  view_only_func = function(tag)
...
local revelation = {
  ...
  border_color = beautiful.revelation_border_color
    or beautiful.border_focus
    or "#DCDCCC",
  border_width = beautiful.revelation_border_width
    or beautiful.border_width
    or 2,
  hintsize = (
    type(beautiful.xresources) == "table"
      and beautiful.xresources.apply_dpi(beautiful.revelation_hintsize or 50)
    or 60
  ),
}

-- Executed when user selects a client from expose view.
local function selectfn(t, zt)
  return function(c)
    ...
    if awful.util.table.hasitem(hintindex, c) then
      ...
      -- Perform the actual focus change
      jump_to_func(c)
    end
  end
end
...
local function match_clients(rule, _clients, t, is_excluded)
  ...
  local mf = is_excluded
      and function(c, _rule)
        return not mfc(c, _rule)
      end
    or mfc
  local flt
...
function revelation.expose(args)
  ...
  for scr = 1, screen.count() do
    t[scr] =
      awful.tag.new({ revelation.tag_name }, scr, awful.layout.suit.fair)[1]
    zt[scr] = awful.tag.new(
      { revelation.tag_name .. "_zoom" },
      scr,
    ...
    if curr_tag_only then
      ...
      match_clients(rule, client.get(scr), t[scr], is_excluded)
    end
  ...
  -- No need for awesome WM 3.5.6: awesome.emit_signal("refresh")
  --
...
local function hintbox_pos(char)
  ...
  hintbox[char].x =
    math.floor(geom.x + geom.width / 2 - revelation.hintsize / 2)
  hintbox[char].y =
    math.floor(geom.y + geom.height / 2 - revelation.hintsize / 2)
end
    ...
    if awful.util.table.hasitem(mod, "Shift") then
      ...
      elseif zoomedClient ~= nil then
        awful.tag.history.restore(
          zoomedClient.screen.index or zoomedClient.screen
        )
        toggle_tag_func(
          zt[zoomedClient.screen.index or zoomedClient.screen],
          zoomedClient
        )
